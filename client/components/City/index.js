...
class City extends React.Component {
  componentDidMount() {
    this.app = application.create('#viewport', {
      autoRender: false,
      devicePixelRatio: 2,
      ...
      init(app) {
        ...
        this._diffuseTex = app.loadTextureSync(
          '//:127.0.0.1:3333/paper-detail.cd627577.png',
          {
            anisotropic: 8
          }
        );
      ...
      methods: {
            ...
            return fetch(url, {
              ...
              .then(buffer => {
                ...
                features.roads = features.roads.filter(feature => {
                  ...
                  return (
                    geoType === 'LineString' || geoType === 'MultiLineString'
                  );
                });
        ...
        generateClouds(app) {
          ...
          for (let i = 0; i < cloudNumber; i++) {
            ...
            const cloudMesh = app.createMesh(
              geo,
              cloudMaterial,
              this._cloudsNode
            );
